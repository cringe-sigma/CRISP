...
## Raspberry Pi 5 raspi5 branch

Run on the Raspberry Pi 5 board:

```bash
git clone --branch raspi5 https://github.com/cringe-sigma/CRISP.git
cd CRISP

bash crisp_run_all_pi5_paper.sh --install-deps --paper --with-closure
```

Quick smoke run:

```bash
bash crisp_run_all_pi5_paper.sh --repeats 100 --bench fac jfdctint
```
...
```bash
bash crisp_run_all_pi5_paper.sh --install-deps --paper --with-closure --allow-exclusions
```

Expected outputs:

```text
paper_runs/pi5/<run-id>/crisp_data/platform_manifest.csv
paper_runs/pi5/<run-id>/crisp_data/taclebench_manifest.csv
paper_runs/pi5/<run-id>/crisp_data/e24_pair_additivity_summary.csv
paper_runs/pi5/<run-id>/crisp_data/e24_pair_additivity_max_summary.csv
paper_runs/pi5/<run-id>/crisp_data/e23_mix_amplification_summary.csv
paper_runs/pi5/<run-id>/crisp_data/rho_safe_closure_summary.csv
paper_runs/pi5/<run-id>/results_manual/e23/*.txt
paper_runs/pi5/<run-id>/results_manual/e24/*.txt
paper_runs/pi5/<run-id>/results_manual/samples/*.csv
paper_runs/pi5/latest -> <run-id>
```
